...
Now for identifiers and keywords. To implement a word outside of a quote, we need to check if our next character is alphanumeric. For simplicity's sake I guess we aren't allowing variable names and other identifiers to start with numbers, because we'll be checking for an identifier using a `isAlpha()` method that does the same kind of string operator Java native nonsense the `isDigit` method does. We'll also stuff this in the default case of our matcher in the scanToken function. If this flag triggers, we're gonna trigger an `identifier()` method that will loop through the remaining alphanumeric characters, advancing/consuming them until we get to the end of the identifier. Here's where things get kind of cool. We're gonna have a static hashmap in the root of this class that contains all the keywords that are native to our interpreted programming language (appropriately named `keywords`), and store the string usage as a key, and the TokenType as the value. Now that we have this, in our `identifier()` method, we can query the text substring against our hashmap of keywords. If it's in the Hashmap, we'll assign the token the respective value, if not, we'll assign `TokenType.IDENTIFIER`.